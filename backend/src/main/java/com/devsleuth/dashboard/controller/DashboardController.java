...
public class DashboardController {
    ...
    public ResponseEntity<List<PullRequest>> getRecentPRs(@RequestParam UUID repositoryId, HttpSession session) {
        accessGuard.requireRepository(repositoryId, (UUID) session.getAttribute("userId"));
        return ResponseEntity.ok(pullRequestRepository.findByRepositoryIdOrderByCreatedAtDesc(repositoryId));
    }

    @GetMapping("/pull-requests/{prId}/reviews")
    public ResponseEntity<List<Review>> getReviews(@PathVariable UUID prId, HttpSession session) {
        accessGuard.requirePullRequest(prId, (UUID) session.getAttribute("userId"));
        return ResponseEntity.ok(reviewRepository.findByPullRequestIdOrderByCreatedAtDesc(prId));
    }
}
